...
function Home(props) {
	...
	async function handleClick(e) {
		e.preventDefault();
		try {
			const response = await fetchData(`/api/${username}`);
			setData(response);
			setResult(true);
		} catch (error) {
			console.error(error);
		...
		<>
			{!result && (
				<Box
					...
					<Header
						text='Welcome to '
						styledText='MyTweeyyAPI😇'
						content='MyTweepyAPI provide the testing twitter apis with real account. ex:] you can search your most recent tweets
							and find the most liked.'
					/>
					<Card
						sx={{
							p: 4,
